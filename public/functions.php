...
// the_post_thumbnailの出力をフィルターでカスタマイズ
add_filter('post_thumbnail_html', 'add_sold_out_label_to_hardware_thumbnail', 10, 5);

function add_sold_out_label_to_hardware_thumbnail($html, $post_id, $post_thumbnail_id, $size, $attr) {

  // 現在の投稿タイプが 'hardware' であるか確認
  if (get_post_type($post_id) === 'hardware') {

    // ACFの販売終了フラグを取得
    $is_sold_out = get_field('hardware-soldout', $post_id);

    // 販売終了フラグが立っている場合はラベルを追加
    if ($is_sold_out) {
      $label = '<div class="soldout-label">販売終了</div>';
      // サムネイル画像の親要素にラベルを追加
      $html = $html . $label;
    }
  }
